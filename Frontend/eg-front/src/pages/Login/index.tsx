...
const validationSchema = Yup.object({
  ...
  password: Yup.string().min(1, "Password is required").required("Required"),
});
...
export default function Login() {
  ...
  useEffect(() => {
    if (state.isAuthenticated || localStorage.getItem('user')) {
      navigate('/');
    }
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      localStorage.setItem("user", JSON.stringify({ name: "Faraz" }));
      dispatch(login({ name: "Faraz", email: '' }))
      ...
      setShowToast({
        show: true,
        message: err.message || "Error while signing in user",
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-start">
                    <div className="flex items-center h-5">
                      <Checkbox name="remember" id="remember" type="checkbox" />
                    </div>
                  ...
                  <a
                    href="/"
                    className="text-sm font-medium text-orange-600 hover:underline dark:text-gray-500"
                  >
                    Forgot password?
                  </a>
                </div>
